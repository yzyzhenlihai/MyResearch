...
from src.core.diffusion.elucidated_diffusion import ElucidatedDiffusion
from src.core.diffusion.data.norm import BaseNormalizer
from src.core.envs.DiffusionEnv import DiffusionEnv # 导入定义的用于扩散模型训练的推荐系统环境
...
def construct_diffusion_model(
    ...
    for k, v in data_shape.items():
        T, d = v
        D += d
    # instantiate方法通过自动读取.yaml配置文件，来实例化一个对象
    # 使用.yaml特殊的建_target_来指向python类的完整路径
...
def get_diffusion_env(args, ensemble_models):
    ...
    diffusion_env = DiffusionEnv(user_emb_dim, item_emb_dim, args.use_userEmbedding)

    return diffusion_env
