...
from src.core.diffusion.denoiser_network.helpers import (
    SinusoidalPosEmb, 
...
class MixerBlock(nn.Module):
    ...
    def forward(self, x, time_emb):
        ...
        '''
        bs, ch, height, width = x.size() # height对应序列长度，width对应特征维度
...
class MixerUnet(nn.Module):
    ...
    def forward(self, x, time, cond):
        ...
        if cond is not None and cond.dtype == torch.float64:
        ...
        if self.cond_dim > 0:
            if cond is not None:
                ...
                """
                在训练时 (self.use_dropout 为 True)，它有 condition_dropout (例如 25%) 的概率将 cond 向量随机置为全零。
                这迫使同一个网络既要学会有条件生成（当 cond 不为零时），也要学会无条件生成（当 cond 为零时）。
                这样，在采样时我们就可以利用这两者的差值来做强力引导。
                """
                if self.use_dropout : 
            ...
            t = torch.cat([cond, t], dim=-1) # 注入条件，将条件向量cond和时间向量t拼接在一起
