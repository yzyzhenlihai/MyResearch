...
from src.core.diffusion.data.norm import BaseNormalizer
...
class ElucidatedDiffusion(nn.Module):
    def __init__(
        ...
        假设
        data_shape = {
            'observations': (16, 24),  # T=16, 观测维度d=24
            'actions': (16, 8)         # T=16, 动作维度d=8
        }
        """
        self.data_shape = data_shape
        self.action_dim = data_shape['actions'][1] # 动作维度
        self.obs_dim = data_shape['observations'][1] # 观测维度
        D = 0 # 总维度，将data_shape中的所有维度进行拼接算出总维度
        for k, v in data_shape.items():
            T, d = v
            D += d
        self.event_shape = [T, D] # 事件形状，即时间步长T和总维度D，也是扩散模型学习生成的数据单元

        # parameters
        self.sigma_min = sigma_min  # 定义噪声水平的最小和最大范围
        self.sigma_max = sigma_max  
        self.sigma_data = sigma_data    
        self.rho = rho  # 控制采样过程中噪声水平的下降速度
        self.P_mean = P_mean # 控制训练时随机采样的噪声水平的分布。
        self.P_std = P_std
    ...
    """
        网络向前传播
        这个方法将上面的预处理函数应用起来。它接收带噪声的输入 noised_inputs 和噪声水平 sigma，
        然后用 c_in 缩放输入，用 c_noise 编码噪声水平，调用神经网络 self.net，
        最后用 c_skip 和 c_out 组合网络输出和原始输入，得到最终的去噪结果。
    """
    ...
    """
        根据 sigma_min, sigma_max, 和 rho 生成一个从高到低的噪声水平列表。
        模型在生成新数据时，会按照这个列表一步步去除噪声。
    """
    ...
    def sample(
        ...
        for sigma, sigma_next, gamma in tqdm(sigmas_and_gammas, desc='sampling time step', mininterval=1,
            ...
            eps = self.S_noise * torch.randn(shape, device=self.device)  # stochastic sampling 从标准正态分布中采样初始噪声

            sigma_hat = sigma + gamma * sigma 
            inputs_hat = inputs + math.sqrt(sigma_hat ** 2 - sigma ** 2) * eps # 添加额外噪声以增加随机性

            denoised_over_sigma = self.score_fn(inputs_hat, sigma_hat, clamp=clamp, cond=cond) # 计算去噪方向
            inputs_next = inputs_hat + (sigma_next - sigma_hat) * denoised_over_sigma

            # second order correction, if not the last timestep
            if sigma_next != 0:
                denoised_prime_over_sigma = self.score_fn(inputs_next, sigma_next, clamp=clamp, cond=cond) # 为提高精度，再次评估去噪方向
                
                
                inputs_next = inputs_hat + 0.5 * (sigma_next - sigma_hat) * (
                        denoised_over_sigma + denoised_prime_over_sigma) # 将预测步和校正步得到的两个方向进行平均
    ...
    def sample_back_and_forth(
        ...
        noise_ts = int(num_sample_steps*(1-noise_level)) # 计算噪声时间步
        ...
        for sigma, sigma_next, gamma in tqdm(sigmas_and_gammas[noise_ts:], desc='sampling time step', mininterval=1,
                                             disable=disable_tqdm): # 不会从0（最大噪声开始），而是从noise_ts开始
            sigma, sigma_next, gamma = map(lambda t: t.item(), (sigma, sigma_next, gamma))

            eps = self.S_noise * torch.randn(shape, device=self.device)  # stochastic sampling

            sigma_hat = sigma + gamma * sigma
            inputs_hat = inputs + math.sqrt(sigma_hat ** 2 - sigma ** 2) * eps # 为了增加随机性，添加额外噪声

            denoised_over_sigma = self.score_fn(inputs_hat, sigma_hat, clamp=clamp, cond=cond) # 有条件预测
            uncond_denoised_over_sigma = self.score_fn(inputs_hat, sigma_hat, clamp=clamp, cond=None) # 无条件预测
            # wo cond
            ...
            # apply condition，硬约束，确保生成的新轨迹一定从这个状态开始
            if cond_state is not None:
    ...
    def score_fn(
        ...
        denoised_over_sigma = (x - denoised) / sigma # 计算去噪方向
    ...
    def forward(self, inputs, cond=None):
        ...
        T, D = self.event_shape
        sigmas = self.noise_distribution(batch_size) # 采样噪声水平，依赖P_mean和P_std这两个超参数
        padded_sigmas = sigmas.reshape(batch_size, 1, 1).repeat(1, T, D)

        noise = torch.randn_like(inputs)
        noised_inputs = inputs + padded_sigmas * noise  # alphas are 1. in the paper 构造带噪样本
        denoised = self.preconditioned_network_forward(noised_inputs, sigmas, cond=cond) # 得到网络的去噪预测

        mask = torch.ones_like(inputs).to(self.device)
        mask[:, -1, self.obs_dim:] = 0.       

        losses = F.mse_loss(denoised, inputs, reduction='none') # 计算损失
        losses = losses * mask
        ...
        if dataset is not None:
            ...
            if reweighted_training:
                ...
                dl = DataLoader(dataset, 
                                ...
                                pin_memory=True, 
                                num_workers=0)
                dl = self.accelerator.prepare(dl) # accelerator会自动移动到正常设备，设置混合精度
                self.dl = cycle(dl)
        ...
        with tqdm(initial=self.step, total=self.train_num_steps, disable=not accelerator.is_main_process) as pbar:
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    states, actions, rewards, next_states, returns, time_steps, terminals, rtg, user_id= next(self.dl) # _这里是数据集中记录的userId
                    if self.discounted_return:
                        cond = returns[:,0,:].to(device, dtype=torch.float32) #float32类型
                    else:
                        trajectory_reward = rewards.squeeze().sum(dim=-1)
                        cond = trajectory_reward.reshape(-1, 1).to(device, dtype=torch.float32)
                    ...
                    last_transition = last_transition.to(device, dtype=torch.float32)   
                    data = torch.cat(data, dim=-1).to(device, dtype=torch.float32)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        self.save(self.step) # 保存当前步数下的扩散模型文件
