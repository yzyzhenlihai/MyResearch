...
class DiffusionDataset(Dataset):
    ...
    # 重写getitem方法，返回一个固定长度为seq_len的子轨迹
    def __getitem__(self, index):
        traj = self.dataset[index]
        traj_len = traj["observations"].shape[0]
        start_idx = random.randint(0, traj_len - self.seq_len) # 随机选择一个起始位置，然后获得子轨迹
...
class DiffusionTrajectoryDataset(DiffusionDataset):
    def __init__(
        ...
        self.seq_len = seq_len # 目标子序列长度
        self.discounted_return = discounted_return
        self.dataset_nm = dataset_nm # 数据集名称 
        self.restore_rewards = restore_rewards
        penalty=100 if penalty is None else penalty
        self.penalty = penalty
        # 保存
    ...
    def _load_d4rl_trajectories(
        ...
        """
        将原始轨迹数据转化为固定固定长度的子轨迹集合，并汇总统计信息
        trajectory_rewards、episode_rewards、trajectory_returns、terminal_idx、terminal_rate供后续使用
        """
        traj, traj_len = [], []
        episode_rewards = [] # 记录每条episode的累计回报
        terminal_idx = [] 
        terminal_cnt = 0
        non_terminal_cnt = 0
        for episode_data in tqdm(dataset) : # 遍历每一条episode（完整轨迹）
            if len(episode_data["observations"]) < self.seq_len:
                continue
            # 计算轨迹的RTG，return to go，计算从当前时刻t到轨迹计算结束的累计奖励（剩余累计奖励）
            episode_data["RTG"] = self._discounted_cumsum(
                episode_data["rewards"], gamma=1.0
            )
            ...
            if self.discounted_return and ("maze" not in self.dataset_nm):
                ...
                if episode_data["terminals"].any():
                    assert (episode_data["terminals"][-1]) and (not episode_data["terminals"][:-1].any())
                    episode_data["rewards"][-1] -= self.penalty # 惩罚最后一个奖励
            # 计算折扣累计回报
            episode_data["returns"] = self._discounted_cumsum(
                episode_data["rewards"], gamma=gamma
            )
        ...
        trajectory_rewards = []
        trajectory_returns = []
        for tr, tlen in zip(traj, traj_len): # 遍历每一条完整轨迹
            for traj_idx in range(tlen - self.seq_len + 1): # 提取子轨迹
                start_idx = traj_idx
                ...
                # 在每条子轨迹中保存user_id
                tr_user_id = tr.get("user_id", -1) if isinstance(tr, dict) else -1
                subtrajectories.append([states, actions, rewards, next_states, returns, time_steps, terminals, rtg, tr_user_id])
        ...
        return subtrajectories, info # 返回子轨迹以及子轨迹的一些统计信息
