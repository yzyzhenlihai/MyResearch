...
def get_GTA_dataset(env: str, step: Optional[int]=None) -> Dict:
    #data = np.load(f'data/generated_data/{env}.npz', allow_pickle=True) # data는 Dict의 list로 되어 있다. 原代码
    data = np.load(f'data/generated_data/{env}/gta_smaples.npz', allow_pickle=True) 
    ...
    #metadata['guidance_target_multiple'] = config_dict['SimpleDiffusionGenerator']['guidance_rewardscale'] # config里没有step配置，应该是amplify_returnscale
...
def get_dataset(config):
    if config.GDA is None or config.GDA == 'None':
        ...
        #generated_dataset, metadata = get_GTA_dataset(config.env, config.step) # 原代码，但是config没有setp配置
