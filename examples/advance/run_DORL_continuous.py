...
def _activate_swanlab_metric_logger():
    """把 trainer/评估链路中的实验日志后端切换到 swanlab。

    `policy_utils` 与 `loggers` 历史上通过名为 `wandb` 的模块变量写入指标，
    默认加载的是真正的 wandb 包，其 run 从未初始化。这里在 swanlab.init 之后
    把这些模块变量指向 swanlab，否则只有 run 初始化、看不到每轮评估曲线。
    """

    for module_name in (
        "policy_utils",
        "src.core.evaluation.loggers",
    ):
...
def get_args_DORL():
    ...
    parser.add_argument("--model_name", type=str, default="DORL_continuous")
    ...
    # 连续动作到离散 item 映射时的探索率，RecPolicy 的 continuous 分支会用到。
    ...
    parser.add_argument("--message", type=str, default="DORL_continuous")
...
def setup_policy_model(args, state_tracker, train_envs, test_envs_dict):
    ...
    # model：连续动作版本，actor 输出与 item embedding 同维度的连续向量，
    # 再由 RecPolicy 的 continuous 分支通过 get_score + select_action 映射为离散 item id。
    net = Net(args.state_dim, hidden_sizes=args.hidden_sizes, device=args.device)
    actor = ActorProb(net, state_tracker.emb_dim, unbounded=True, device=args.device).to(args.device)
    critic = Critic(
